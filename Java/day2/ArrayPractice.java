...
//    (배열을 직접 수정하는 swap은 아래 메서드로 구현)
    static void swap(int[] arr, int i, int j) {
        int temp = arr[i]; // 임시저장
        arr[i] = arr[j]; // i 자리에 j 값 덮어쓰기
        arr[j] = temp; // j 자리에 임시값(원래 i 값) 저장
    }
...
//    추가 배열 없이 원본 배열 자체를 뒤집는다. 메모리가 더 효율적으로 쓰임
...
//    오른쪽 k칸 회전 = 왼쪽 (n - k)칸 회전과 동일!
    ...
    //    중복 제거
//    정렬 후 인접한 중복 제거
//    정렬하면 같은 값이 붙어 있으므로 이전 값과 비교해서 다를 때문 추가한다.
...
//        두 변수의 값 교환부터 중복 제거까지 배열을 만들어서 스스로 출력해보세요~~
...
//        TODO 실습문제
        /*
         * 1. 배열 {1, 2, 3, 4, 5, 6, 7, 8}을 3칸씩 오른쪽으로 회전하여 출력하세요
         *
         * 심화문제
