...
//        1. 배열이란?
        /*
         * 같은 자료형의 데이터를 연소된 메모리 공간에 저장하는 자료구조.
         * 변수 하나로 여러 개의 값을 관리할 수 있다.
         * 인덱스는 0부터 시작한다!
         * 크기는 한 번 정해지면 변경할 수 없다.
         *
         * 2. 배열 선언과 초기화 방법3가지
         * 방법 1: 크기만 지정 (값은 자동으로 기본값으로 채워짐)
...
//        Arrays.copyOfRange() : 범위를 지정해서 복사 (from 포함, to제외)
        int[] ranged = Arrays.copyOfRange(original, 1, 4); //인덱스1~3
...
//        TODO 실습 문제
         ...
         * 코드를 작성해보세요 (힌트 : 반복 카운트, 이중 포문)
         * 3. 조금 어려움 주의. 배열을 오름차순 정렬sort 없이 직접 오름차순 정렬을
         * 구현해보세요. (인정한 두 값을 비교해서 교환하는 방식 -> 버블 정렬, temp)
         *
         * */
...
//        1.
        ...
        System.out.println("");
//        2.
        int[] freqArr = {3, 1, 4, 1, 5, 9, 2, 6};
        int modeValue = freqArr[0]; // 최빈값 (초기값 : 첫 번째 요소)
        int modeCount = 0; //최빈값의 등장 횟수

//        각 요소를 기준으로 전체 배열에서 몇 번 등장하는 센다
        for (int i = 0; i < freqArr.length; i++) {
            int count = 0;
            for (int j = 0; j < freqArr.length; j++) {
                if (freqArr[j] == freqArr[i]) {
                    count++;
                }
            }
//            현재 요소의 빈도가 최고 빈도보다 높으면 갱신
            if (count > modeCount) {
                modeCount = count;
                modeValue = freqArr[i];
            }
        }
        System.out.println("최반값: " + modeValue + " " + modeCount + "번 등장");
//          3. 버블정렬 풀이 다른 방법도 많음!
