...
//        Book 생성자 내부에서 this.title, this.author, this.price 로 구분
//        bookDefault 는 this("미상",0) 으로 다른 생성자를 위임 호출함
...
* - 계좌 A (잔액 50000) -> 계좌 B(잔액 20000) 로 30000원 이체 후 각 잔액 출력
...
//    생성자 오버로딩 1 - 제목만 입력 (나머지 기본값)
    public Book(String title) {this(title, "미상",0);} // this()로 아래 생성자 위임 호
...
//    할인율(%)을 적용한 가격 반환
    public int discountPrice(int discountPercent) {return price * (100-discountPercent) / 100;}
