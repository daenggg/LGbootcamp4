/*
 * 인터페이스와 추상 클래스
 * 인터페이스의 역할과 사용방법을 이해한다
 * 추상페이스의 역할과 사용방법을 이해한다
 * 인터페이스와 추상클래스의 차이점을 비교한다
 * 다형성을 활용한 설계 방식을 이해한다
 * */
...
//        일반 메서드 : 자식이 그대로 쓰거나 오버라이딩할 수 있다.
//        interface와 달리 필드와 생성자를 가질 수 있다.
        ...
        Document d1 = new Document("책1");
        Document d2 = new Document("책2");
        d1.print();
        d2.print();
    }
...
class Document implements printable {
    ...
    @Override
    public void print() {
        System.out.println("문서 출력 중: " + title);
    }
}
...
class IntfCircle extends AbstractShape {
    ...
    public IntfCircle(double r, String color) {
        super(color);
        this.r = r;
    }
...
class IntfRect extends AbstractShape {
    ...
    public IntfRect(double w, double h) {
        super();
        this.w = w;
        this.h = h;
    }
...
class IntfTriangle extends AbstractShape {
    ...
    public IntfTriangle(double base, double height) {
        super();
        this.base = base;
        this.height = height;
    }
