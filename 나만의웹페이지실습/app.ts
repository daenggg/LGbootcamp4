    ...
    const render = (category: Category) => {
        ...
        clothesGrid.innerHTML = displayData
            ...
            <article class="clothes-card">
                <div class="clothes-img" style="font-size: 50px; text-align: center; padding: 20px;">
                    ${item.image}
                </div>
                <div class="clothes-info" style="padding: 10px; border-top: 1px solid #eee;">
                    <h3 style="font-size: 16px; margin: 5px 0;">${item.name}</h3>
                    <p style="font-weight: bold; color: #ff4d7d;">${item.price.toLocaleString()}원</p>
                </div>
            </article>
        `,
            )
